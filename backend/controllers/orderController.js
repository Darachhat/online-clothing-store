...
const BAKONG_API_URL = 'https://api-bakong.nbc.org.kh/v1/payment';
const BAKONG_API_KEY = 'test_bakong_api_key_placeholder';
const BAKONG_MERCHANT_ID = 'test_bakong_merchant_id';
...
export const bakongOrder = async (req, res) => {
  try {
    // Destructure request body
    const { userId, items, amount, address, paymentMethod } = req.body;
    ...
    const transactionId = `BAKONG_${req_time}_${userId}`;
    const { firstName, lastName, phone, email, street, city, country } = address;
    ...
    const currency = "USD";
    const payment_option = "";
    const shipping = (items.length * 0.2).toFixed(2);

    // Generate hash for Bakong (using similar pattern as ABA)
    const hashInput = `${req_time}${BAKONG_MERCHANT_ID}${transactionId}${amount}${itemsBase64}${shipping}${firstName}${lastName}${email}${phone}${type}${payment_option}${currency}${return_params}`;
    const hash = getHash(hashInput);
    ...
    console.log("Bakong Order created:", order);

    // Generate HTML form for Bakong payment
    res.send(`
      <form method="POST" action="${BAKONG_API_URL}" id="bakong_merchant_request">
        <input type="hidden" name="hash" value="${hash}" />
        <input type="hidden" name="tran_id" value="${transactionId}" />
        <input type="hidden" name="amount" value="${amount}" />
        <input type="hidden" name="firstname" value="${firstName}" />
        ...
        <input type="hidden" name="currency" value="${currency}" />
        <input type="hidden" name="type" value="${type}" />
        <input type="hidden" name="merchant_id" value="${BAKONG_MERCHANT_ID}" />
        <input type="hidden" name="req_time" value="${req_time}" />
        <input type="hidden" name="return_params" value="${return_params}" />
        <input type="hidden" name="payment_option" value="${payment_option}" />
      </form>
      <script>
        document.getElementById('bakong_merchant_request').submit();
      </script>
    `);
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: "An error occurred during Bakong payment initiation.",
    });
  }
}

// Verify Bakong Payment
export const verifyBakong = async (req, res) => {
  const { orderId, success, userId } = req.body;
